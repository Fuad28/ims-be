...
class Order(BusinessTimeAndUUIDStampedBaseModel):
    ...
    def compute_qdp(self) -> float | Literal[0]:
        ...
        for item in order_items:
            qdp += QUALITY_PERC * (
                item.qty_defected / item.qty_ordered
            ) + DELIVERY_PERC * (item.qty_delayed / item.qty_ordered)
...
class OrderItem(BusinessTimeAndUUIDStampedBaseModel):
    order = models.ForeignKey(
        Order, on_delete=models.CASCADE, related_name="_order_items"
    )
    product_item = models.ForeignKey(
        ProductItem, on_delete=models.CASCADE, related_name="orders"
    )
