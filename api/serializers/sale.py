...
class SaleSerializer(serializers.ModelSerializer):
    total_selling_price= serializers.DecimalField(max_digits=10, decimal_places=2, read_only= True)
    ...
    def validate_sale_items(self, sale_items: list):
        if not len(sale_items):
            raise serializers.ValidationError("order must contain at least one sale_item")
        ...
        for sale_item in sale_items:
            ...
            if product_item.quantity < quantity:
                raise serializers.ValidationError(f"Not enough quantity of {product_item.name} available")
